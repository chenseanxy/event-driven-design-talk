((Reveal, Expo, TimelineMax) => {
    ...
    Reveal.addEventListener('event-loop-in-action', () => {
        ...
        timeline.to(runTask, 0.2, {className: '+=current'})
            ...
            .to(queuedTimer, 0.5, {x: '-=1000', ease: Expo.easeIn})
            .to(queuedTimer, 0.5, {className: '+=disappear'})
            .to(timerTask, 0.5, {className:'-=appear', ease: Expo.easeIn}, '-=0.65')
            .addPause()
            .to(runInNewThread, 0.2, {className: '+=current'})
            .addPause()
            .to(timerTask, 0.5, {className: '+=appear', ease: Expo.easeOut})
            .to(threadLine1, 0.2, {className: '+=current'})
            .addPause()
            .to(threadLine1, 0.2, {className: '-=current'})
            .to(threadLine2, 0.2, {className: '+=current'})
            .addPause()
            .to(queuedCallback, 0.5, {className: '-=appear', ease: Expo.easeIn})
            .to(newThread, 0.2, {opacity: 0})
            .addPause()
            .to(loopLine2, 0.2, {className: '-=current'})
            .to(runTask, 0.2, {className: '+=current'})
            .addPause()
            .to(queuedCallback, 0.5, {x: '-=1000', className: '+=disappear'})
            .to(callbackTask, 0.5, {className: '-=appear', ease: Expo.easeIn})
            .addPause()
            .to(runTask, 0.2, {className: '-=current'})
            .to(invokeCallback, 0.2, {className: '+=current'});

        mainLoop.to(loopLine2, 0.2, {className: '-=current'}, 0)
            .to(loopLine1, 0.2,  {className: '+=current'}, 0)
            .to(loopLine1, 0.2,  {className: '-=current'}, 2)
            .to(loopLine2, 0.2,  {className: '+=current'}, 2);
